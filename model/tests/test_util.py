...
def test_temporal_filter() -> None:
    tf = DoubleExponentialFilter(device=device, tau_rise=1, tau_fall=1, dt=1)
...
def test_spike_moving_average() -> None:
    ...
    assert sma.apply(
        spike=torch.Tensor(
            [1])).item() == 0.6321205496788025

    # Apply another spike, the average should increase
    assert sma.apply(spike=torch.Tensor([2])).item() == 1.496785283088684

    # After some time with no spikes, the average should decay
    assert sma.apply(
        spike=torch.Tensor(
            [0])).item() == 0.5506365299224854
...
def test_variance_moving_average() -> None:
    ...
    sma = SpikeMovingAverage(
        ...
        tau_mean=tau_mean,
        dt=1)
    vma = VarianceMovingAverage(device=device, tau_var=tau_var, dt=1)
...
def test_inhibitory_plasticity_trace() -> None:
    ...
    ipt = InhibitoryPlasticityTrace(device=device, trace_shape=trace_shape, dt=0.1)

    # Apply a single spike
    assert ipt.apply(spike=torch.Tensor([[1]])).item() == 1

    # Apply another spike, the trace should increase
    assert ipt.apply(spike=torch.Tensor([[2]])).item() > 2

    # After much time with no spikes, the trace should decay
    assert ipt.apply(spike=torch.Tensor([[0]])).item() > 0
